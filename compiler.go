...
func (c *compiler) compileComma(e *Comma) error {
	...
	if err := c.lazyCode(
		func() (*code, error) { return &code{op: opfork, v: len(c.codes) + 1}, nil },
		func() error { return c.compileComma(&Comma{e.Alts[:len(e.Alts)-1]}) },
	); err != nil {
		return err
	}
	return c.lazyCode(
		func() (*code, error) { return &code{op: opjump, v: len(c.codes) - 1}, nil },
...
func (c *compiler) compileAlt(e *Alt) error {
	...
	if err := c.lazyCode(
		func() (*code, error) {
			return &code{op: opfork, v: len(c.codes) + 7}, nil // opload found
		},
	...
	c.append(&code{op: opjumpifnot, v: len(c.codes) + 3}) // oppop
	c.append(&code{op: oppush, v: true})                  // found some value
	c.append(&code{op: opstore, v: found})
	return c.lazyCode(
		func() (*code, error) {
			return &code{op: opjump, v: len(c.codes) - 1}, nil // ret
		},
		func() error {
			c.append(&code{op: oppop})
			c.append(&code{op: opbacktrack})
			c.append(&code{op: opload, v: found})
			c.append(&code{op: opjumpifnot, v: len(c.codes) + 2})
...
func (c *compiler) compileIf(e *If) error {
	...
	if err := c.lazyCode(
		func() (*code, error) {
			return &code{op: opjumpifnot, v: len(c.codes)}, nil // if falsy, skip then clause
		},
		func() error {
	...
	return c.lazyCode(
		func() (*code, error) {
			return &code{op: opjump, v: len(c.codes) - 1}, nil // jump to ret after then clause
		},
		func() error {
			c.append(&code{op: opload, v: idx})
			if len(e.Elif) > 0 {
...
func (c *compiler) compileArray(e *Array) error {
	...
	return c.lazyCode(
		func() (*code, error) {
			return &code{op: opfork, v: len(c.codes) - 1}, nil
		},
		func() error {
...
func (c *compiler) compileIter() error {
	...
	c.append(&code{op: opjumpifnot, v: len(c.codes) + 7}) // oppop
	c.append(&code{op: opfork, v: len(c.codes) - 4})      // opload length
	c.append(&code{op: opload, v: idx})
	c.append(&code{op: opindex})
	c.append(&code{op: opload, v: idx})
	c.append(&code{op: opincr})
	c.append(&code{op: opstore, v: idx})
	c.append(&code{op: opjump, v: len(c.codes) + 2})
	c.append(&code{op: oppop})
	c.append(&code{op: opbacktrack})
	return nil
}
