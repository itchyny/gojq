...
- gojq supports reading from YAML input (`--yaml-input`) while jq does not. gojq also supports YAML output (`--yaml-output`). gojq supports a few filters missing in jq; `scan/2` ([jq#2207](https://github.com/stedolan/jq/pull/2207)), and `@urid` format string ([jq#2261](https://github.com/stedolan/jq/issues/2261)).
